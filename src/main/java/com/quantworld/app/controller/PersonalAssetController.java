...
public class PersonalAssetController {
  ...
  @RequestMapping(value = "/getPersonalAssetInfo/{exchangeName}", method = RequestMethod.GET)
  @LoggerManage(description = "加载合约查询页面")
  public Response getAssetInfo(@PathVariable String exchangeName) {
    // 1. exchange query
    // 2. existing Asset updating time/ number
    JSONObject data;
    if (ALL.equals(exchangeName)) {
      data = updateAllExchangePersonalAsset();
    } else {
      data = updateSpecificExchangePersonalAsset(exchangeName);
    }
  ...
  private JSONObject updateSpecificExchangePersonalAsset(String exchangeName) {
  ...
  private JSONObject updateAllExchangePersonalAsset() {
    ...
    InternalUtils.await(1000);
    List<AccountData> accountDataList = orderManagementSystem.getAllAccountsData();
    Map<String, Integer> exchangeContractNumMap = new ConcurrentHashMap<>();
    for (AccountData accountData : accountDataList) {
      String exchangeName = accountData.getExchange().name();
      accountData.getAsset();
      accountData.getBalance();
      accountData.getFrozen();
